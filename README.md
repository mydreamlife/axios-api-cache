...
window.apiCatchInstance.interceptorsRequest(
  config => {
    return config
  },
  () => { // 异常回调处理，如果没有，则可以不传
...
window.apiCatchInstance.interceptorsResponse( // 如果业务没有响应拦截处理，此方法可以无需定义
  response => {
...
export default window.apiCatchInstance
```